...
t_tok	*creat_token(int start, int len, char *str)
{
	t_tok	*lst;
...
int	ft_get_great(char *str, int i, t_tok **head)
{
	...
	if (str[i] == '>' && str[i + 1] == '>')
	{
		...
		while (str[i] && str[i] == '>')
			i++;
		ft_add_back(head, creat_token(j, (i - j), str));
		return (i);
	}
	ft_add_back(head, creat_token(i, 1, str));
	return (i + 1);
}
...
int	ft_get_double(char *str, int i, t_tok **head)
{
	...
	if (str[i] == '"' && str[i + 1] == '"')
		ft_add_back(head, creat_token(i, 2, str));
	else
	{
		j = i;
		i = i + 1;
		while (str[i] && str[i] != '"')
			i++;
		ft_add_back(head, creat_token(j, (i - j) + 1, str));
		return (i + 1);
	}
	return (i + 2);
}
